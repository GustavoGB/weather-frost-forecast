...
**Função de perda.** O GBT vencedor minimiza **log loss** (perda logística / desvio binomial — em Spark, `GBTClassifier` só suporta `lossType='logistic'`), assim como a Regressão Logística; o RandomForest, por contraste, cresce árvores por **impureza de Gini**. Como log loss é uma *proper scoring rule*, o modelo aprende **probabilidades calibradas**, não apenas uma classe 0/1 — é o que viabiliza o *Frost-Risk Score* e o ajuste de limiar. Vale distinguir três objetivos: **treino** minimiza log loss, a **seleção** de hiperparâmetros usa **ROC-AUC** (§7.4), e a **decisão** final move o limiar pelo custo de negócio (FN ≫ FP, §8.5).
...
**A subamostra de 10% custa qualidade? Não — verificamos.** Treinar o mesmo GBT em **100% do treino** (13,2 M linhas, ~3,7 min nesta máquina de 16 GB) dá **ROC-AUC 0,9742** contra **0,9743** com 10% (1,3 M linhas) — idêntico dentro do ruído (Δ < 0,001 em todas as métricas). O sinal **satura** bem antes de 1,3 M estação-dias (geada é muito persistente), então a subamostra é só **velocidade**, sem perda de qualidade — e o pipeline treina em 100% sem mudar uma linha.
...
Afinamos os hiperparâmetros do GBT vencedor — `maxDepth` (profundidade das árvores) e `maxIter` (nº de árvores) — com **validação cruzada k-fold**: o `CrossValidator` divide o treino em 3 *folds*, testa cada combinação do grid (`maxDepth ∈ {4, 6}` × `maxIter ∈ {20, 40}` = 4 combinações × 3 folds = **12 treinos**) e fica com a de maior **ROC-AUC** médio.

**Ressalva honesta:** o `CrossValidator` do Spark sorteia os *folds* **aleatoriamente**, ignorando a data — então um fold de treino pode conter dias *posteriores* aos do fold de validação (ex.: treina em 10/mar, valida em 05/mar). É um leve **vazamento temporal** que deixa as notas internas da CV otimistas demais. Mas como ele atinge **todas as combinações por igual**, o *ranking* entre elas quase não muda — por isso a CV serve **só para *escolher*** os hiperparâmetros, nunca para medir desempenho.

O número que reportamos (**ROC-AUC ≈ 0,98**) não vem da CV, e sim do **ano de teste reservado (2024)** — treinado só nos anos anteriores e nunca tocado por nenhum fold —, um split **temporal puro**, sem vazamento. O remédio 100% rigoroso seriam *folds* **encadeados no tempo** (*forward-chaining*: treina 2005–2018 → valida 2019; treina 2005–2019 → valida 2020; …), que o Spark ML não traz nativo. Em uma linha: o vazamento afeta **qual** modelo escolhemos, nunca **o quão bom** dizemos que ele é.
...
- *Treinado numa subamostra (10 %), por velocidade* — **verificamos que isso não custa qualidade**:
  treinar em 100 % (13,2 M linhas) dá ROC-AUC 0,9742 vs 0,9743 com 10 % (Δ < 0,001). As métricas
  finais são no teste 2024 completo, e o pipeline treina em 100 % sem alteração.
